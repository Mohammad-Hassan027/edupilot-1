...
const FROM_EMAIL  = "EduPilot Support <onboarding@resend.dev>"
...
function buildAdminEmail(data: {
  ...
  const categoryLabels: Record<string, string> = {
    technical: "🔧 Technical Issue", billing: "💳 Billing",
      ...
      <table width="560" cellpadding="0" cellspacing="0" style="max-width:560px;width:100%;">
        ...
        <tr>
          <td style="background:#1e2130;padding:32px 36px;border-left:1px solid #2d3148;border-right:1px solid #2d3148;">
            ...
            <table width="100%" cellpadding="0" cellspacing="0" style="margin-bottom:20px;border-collapse:separate;border-spacing:0 4px;">
              ${[
                ["Name",     name],
                ["Email",    email],
                ["Category", catLabel],
            ...
            <div style="margin-top:20px;padding:14px 16px;background:rgba(99,102,241,0.1);border:1px solid rgba(99,102,241,0.25);border-radius:8px;">
              <p style="color:#a5b4fc;font-size:13px;margin:0;">
                💡 Reply to this email to respond directly to <strong>${name}</strong> at <a href="mailto:${email}" style="color:#818cf8;">${email}</a>
              </p>
            </div>
          </td>
        </tr>
...
function buildUserEmail(data: { name: string; category: string; subject: string }): string {
  const { name, category, subject } = data
      ...
      <table width="560" cellpadding="0" cellspacing="0" style="max-width:560px;width:100%;">
        ...
        <tr>
          <td style="background:#1e2130;padding:40px;border-left:1px solid #2d3148;border-right:1px solid #2d3148;">
            <table cellpadding="0" cellspacing="0" border="0" align="center" style="margin-bottom:28px;display:block;text-align:center;">
              <tr><td align="center">
            ...
            <p style="color:#94a3b8;font-size:15px;line-height:1.7;margin:0 0 28px;text-align:center;">
              Hi <strong style="color:#e2e8f0;">${firstName}</strong>, we've received your support request and our team is on it.
              Expect a response within <strong style="color:#a5b4fc;">24 hours</strong>.
            </p>
            ...
            <table width="100%" cellpadding="0" cellspacing="0" border="0" style="margin-bottom:24px;">
              <tr><td style="background:rgba(79,70,229,0.08);border:1px solid rgba(79,70,229,0.2);border-radius:12px;padding:20px 24px;">
                ...
                ${[
                  ...
                  ["📧","Reply to this email to add details", "No need to submit a new ticket."],
                ].map(([icon, title, desc]) => `
            ...
            <p style="color:#64748b;font-size:12px;line-height:1.6;margin:0;text-align:center;">
              Urgent? Call <a href="tel:+916352751256" style="color:#818cf8;">+91 63527 51256</a> or email <a href="mailto:contact@edupilot.ai" style="color:#818cf8;">contact@edupilot.ai</a>
            </p>
          </td>
        </tr>
        <tr>
...
export async function POST(req: NextRequest) {
  try {
    ...
    await sendEmail(
      ADMIN_EMAIL,
      `[EduPilot Help] [${category.toUpperCase()}] ${subject}`,
      buildAdminEmail({ name: name.trim(), email, category, subject, message }),
      `Support ticket from ${name} <${email}>\nCategory: ${category}\nSubject: ${subject}\n\n${message}`,
      email
    )

    await sendEmail(
      email,
      "EduPilot — Your Support Ticket Has Been Received",
      buildUserEmail({ name: name.trim(), category, subject }),
