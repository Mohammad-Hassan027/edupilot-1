...
import { analyzeAttachmentsWithGemini, searchWithTavily, type UploadedAttachment } from "@/lib/ai-tools"
  ...
  try {
    ...
    const aiResponse = attachments.length
      ? await analyzeAttachmentsWithGemini({
          message: cleanMessage || "Please review the uploaded file and help me understand it.",
          attachments,
          webContext,
        })
      : await generateAIResponse(cleanMessage || "Please help me with the uploaded study material.", {
          mode,
          webContext,
        })
    ...
    if (user) {
      ...
      const userContent = cleanMessage || `Uploaded files: ${attachments.map((file) => file.name).join(", ")}`
      ...
      logUsage(
        user.id,
        attachments.length ? "ai_file_analysis" : mode === "web_search" ? "ai_web_search" : "ai_chat",
        "question_asked",
        {
