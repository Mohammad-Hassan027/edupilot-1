...
function AITutorContent() {
  ...
  const [selectedMode, setSelectedMode] = useState<"chat" | "image" | "video" | "web">("chat")
  ...
  const handleModeSelect = (mode: "image" | "video" | "web") => {
    setSelectedMode(mode)

    if (!input.trim()) {
      const starters = {
        image: "Create an educational image about ",
        video: "Generate a short learning video script about ",
        web: "Search the web and explain the latest information about ",
      }
  ...
  return (
    <>
      <div className="flex h-[calc(100vh-4rem)] gap-3 md:gap-4 p-3 md:p-6 overflow-hidden">
        <Card className="hidden lg:flex w-72 xl:w-80 flex-col border-border bg-card flex-shrink-0 overflow-hidden h-full">
            ...
            <div className="max-w-4xl mx-auto space-y-3">
                ...
                <div className="flex flex-wrap gap-2">
                  {selectedFiles.map((file) => (
                    <div
                      ...
                      className="inline-flex items-center gap-2 rounded-full border border-border bg-secondary/60 px-3 py-1 text-xs text-foreground"
                    >
              ...
              <div className="flex flex-wrap items-center gap-2">
                {selectedMode !== "chat" && (
                  <div className="inline-flex items-center gap-2 rounded-full border border-primary/30 bg-primary/10 px-3 py-1 text-xs text-primary">
                    {selectedMode === "image" && <ImageIcon className="h-3.5 w-3.5" />}
                    {selectedMode === "video" && <Video className="h-3.5 w-3.5" />}
                    {selectedMode === "web" && <Globe className="h-3.5 w-3.5" />}
              ...
              <div className="relative flex items-center gap-2">
                <div className="relative flex-1">
                  ...
                  <Input
                    placeholder={selectedMode === "chat" ? "Ask anything you want to learn..." : `Ask EduPilot to help with ${selectedMode} generation...`}
                    value={input}
                    onChange={(e) => setInput(e.target.value)}
                    onKeyDown={(e) => e.key === "Enter" && !e.shiftKey && handleSend()}
                    className="h-14 pl-12 pr-10 bg-secondary border-border rounded-2xl text-sm"
                    disabled={isTyping}
                  />
                  ...
                  <DropdownMenu>
                    <DropdownMenuTrigger asChild>
                      <Button
                        variant="ghost"
                        size="icon"
                        className="absolute left-2 top-1/2 h-9 w-9 -translate-y-1/2 rounded-full text-muted-foreground hover:bg-background/80 hover:text-foreground"
                      >
                        <Plus className="h-5 w-5" />
                      </Button>
                    </DropdownMenuTrigger>
                    <DropdownMenuContent align="start" className="w-60 rounded-2xl border-border bg-popover/95 p-2 backdrop-blur">
                      <DropdownMenuItem className="rounded-xl py-3" onClick={() => handleModeSelect("video")}>
                        <Video className="h-4 w-4" />
                        <span>Video generation</span>
                      </DropdownMenuItem>
                      <DropdownMenuItem className="rounded-xl py-3" onClick={() => handleModeSelect("image")}>
                        <ImageIcon className="h-4 w-4" />
                        <span>Image generation</span>
                      </DropdownMenuItem>
                      <DropdownMenuItem className="rounded-xl py-3" onClick={() => fileInputRef.current?.click()}>
                        <Paperclip className="h-4 w-4" />
                        <span>File upload</span>
                      </DropdownMenuItem>
                      <DropdownMenuItem className="rounded-xl py-3" onClick={() => handleModeSelect("web")}>
                ...
                <Button onClick={handleSend} disabled={!input.trim() || isTyping} size="sm" className="h-12 w-12 shrink-0 rounded-2xl">
