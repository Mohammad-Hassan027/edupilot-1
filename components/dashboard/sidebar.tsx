...
export function DashboardSidebar() {
  ...
  const renderItem = (item: typeof navItems[0]) => {
    ...
    // AI Tutor is allowed for guests but with a "Limited" badge
    const isGuestAITutor  = item.highlight && isLoggedIn === false
      ...
      <Tooltip key={item.label}>
        <TooltipTrigger asChild>
          ...
          >
            ...
            {/* Logged-in: sparkle icon on AI Tutor */}
            {item.highlight && !isActive && !isLocked && !isGuestAITutor && (
              <Sparkles className="ml-auto h-4 w-4 text-primary hidden lg:block" />
            )}
            {/* Guest on AI Tutor: show "Free" badge */}
            {isGuestAITutor && !isActive && (
              <span className="ml-auto hidden lg:inline-flex items-center rounded-full bg-amber-500/15 px-1.5 py-0.5 text-[10px] font-semibold text-amber-500 leading-none">
                Free
              </span>
            )}
