...
interface LogoProps {
  ...
  showText?: boolean
  size?: "sm" | "md" | "lg"
  // If provided, overrides auth check (used in dashboard where we know user is logged in)
  href?: string
}
...
export function Logo({ className, showText = true, size = "md", href }: LogoProps) {
  ...
  useEffect(() => {
    if (href) return   // explicit href wins
    fetch("/api/user/profile")
      .then((r) => setDest(r.ok ? "/dashboard" : "/"))
      .catch(() => setDest("/"))
  }, [href])
  ...
  return (
    <Link href={dest} className={cn("flex items-center gap-2.5 group", className)}>
      <div className={cn(
      ...
      {showText && (
        <div className={cn("flex items-center gap-0.5 font-bold", textSizeClasses[size])}>
